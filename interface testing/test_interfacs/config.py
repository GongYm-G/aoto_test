...
class Config:
    ...
    def __init__(self):
        ...
        self.path1 = r"D:\webdriver\接口基本信息.xls"
    ...
    def open(self):
        ex = xlrd.open_workbook_xls(self.path1)
        ex1 = ex.sheet_by_index(3)
        sheet = ex.sheet_names()[3]
        cop = copy(ex)
        cop1=cop.get_sheet(sheet)
        values = [ex1.row_values(i) for i in range(1, ex1.nrows)]
        for i in range(len(values)):
            a = [i for i in values[i][7::2]]
            b = [i for i in values[i][8::2]]
            di = dict(zip(a, b))
            try:
                di.pop('')
            except KeyError:
